...
//  default render time (X,Y=[0,0], scale=1, steps=16, sysclk=125MHz, res=264x200): 2.28 sec
void __not_in_flash_func(MandelFloat)(u8* dst, float ci)
{
	...
	for (x = 0; x < w; x++)
	{
		...
		SETPIXEL(dst, x, ((i == 0) ? COL_BLACK : g[(steps - i) >> s]));
		if ((x & 1) != 0) dst++;
		cr += sx;
	}
}
