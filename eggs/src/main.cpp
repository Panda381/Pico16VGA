...
void DrawBoard()
{
	...
	for (i = 0; i < BOARDH0; i++)
	{
		for (j = 0; j < BOARDW0; j++)
		{
			...
			x = j*TILEW;
			y = i*TILEH;
			DrawImg(&TilesImg[b*TILEW*(TILEH/2)], x+X0, y+Y0, 0, 0, TILEW, TILEH, TILEW, TILEH);
		}
	}
}
...
{
	// display open image
	DrawImg(OpenImg, X0, Y0, 0, 0, 512, 384, 512, 384);
...
void DispSel()
{
	...
	{
		// copy background
		DrawImg(&FrameImg[y*FRAMEW/2], FRAMEX+X0, y+Y0, 0, 0, FRAMEW, SELECTH, FRAMEW, FRAMEH);

		// draw computer
		if (Comp[play]) DrawImg(ComputerImg, COMPX+X0, y+COMPY+Y0, 0, 0, COMPW, COMPH, COMPW, COMPH);

		// draw selection frame
		if (play == Player) DrawBlit(SelectImg, SELECTX+X0, y+SELECTY+Y0, 0, 0, SELECTW, SELECTH, SELECTW, SELECTH, COL_DARKGRAY);
...
void DispScore()
{
	...
	for (play = 0; play < 2; play++)
	{
		...
		NumBuf[n++] = ' ';
		NumBuf[n++] = 0;
		DrawTextBg(NumBuf, SCOREX+X0, SCOREY+play*FRAMEY+Y0, 1, 1, SCORECOL, SCOREBG);
	}
}
...
void DispWin()
{
	...
	for (play = 0; play < 2; play++)
	{
		...
		NumBuf[n++] = ' ';
		NumBuf[n++] = 0;
		DrawTextBg(NumBuf, WINX+X0, WINY+play*FRAMEY+Y0, 1, 1, SCORECOL, SCOREBG);
	}
}
...
void OpenGame(Bool comp1, Bool comp2)
{
	...
	// copy frame image
	DrawImg(FrameImg, FRAMEX+X0, Y0, 0, 0, FRAMEW, FRAMEH, FRAMEW, FRAMEH);
